...
public class SolicitudDocumentoControlador {
    ...
    public ResponseEntity<SolicitudPruebaProcesoDocumento> subirArchivoPruebasProceso(@RequestPart("info") String info,
    ...
    @PostMapping("/solicitudPruebasProceso/imagen1")
    public ResponseEntity<Object> subirArchivoImagen1(@RequestPart("info") String info, @RequestPart("file") MultipartFile file) throws IOException {
        VerImagenDTO imgBase64 = servicePP.subirImg1(info, file.getBytes(), file.getOriginalFilename(), file.getContentType());
        return ResponseEntity.ok(imgBase64);
    }

    @GetMapping("/solicitudPruebasProceso/{estado}/{orden}/{solicitudId}")
    public ResponseEntity<List<SolicitudPruebaProcesoDocumento>> listarArchivosPruebasProceso(@PathVariable("estado") EstadoSolicitudPP estado,
                                                                                              @PathVariable("orden") OrdenFlujoPP orden,
                                                                                              @PathVariable("solicitudId") Long solicitudId) throws IOException {
    ...
    @GetMapping(value = "/solicitudPruebasProceso/ver/imagen/{id}")
    public ResponseEntity<VerImagenDTO> verPPImagen(@PathVariable("id") Long id) {
        VerImagenDTO data = servicePP.verImg(id);
